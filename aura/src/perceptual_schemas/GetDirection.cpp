/**
 * ==============================================================================
 * \file GetDirection.cpp
 * \brief Provides the definition of the perceptual schema class GetDirection
 ...
 * Constructor
 * -----------------------------------------------------------------
 * additional information are given in the GetDirection.h
 * -----------------------------------------------------------------
*/
GetDirection::GetDirection() : PerceptualSchema(DEFAULT_TOPIC_st), targetRFID_ui(DEFAULT_RFID_i),
                               initialRFID_ui(DEFAULT_RFID_i), fieldKeyBuffer_st(DEFAULT_FIELD_KEY_st) {

    this->getFieldByKeyServiceClient_c = this->nh_c.serviceClient<aal_potential_field_navigation::get_field_by_key>(
            GET_FIELD_BY_KEY_st); // advertise the "get_field_by_key" service to call for vectorfields

    this->checkRFIDServiceClient_c = this->nh_c.serviceClient<aal_potential_field_navigation::check_rfid>(
            CHECK_RFID_st); // advertise the "check_rfid" service to get some information about RFID tags
 ...
 * ---------------------------------------------------------
*/
bool GetDirection::getPerceptualData_b(cv::Mat &vectorfield, uint64_t targetRFID_ui, uint64_t currentRFID_ui,
                                       uint64_t lastRFID_ui, uint64_t initialRFID_ui) {
    ...
    if (fieldKey_st == DEFAULT_FIELD_KEY_st) { // check the field key
        ...
        srv_c.request.fieldNamePostfix_st = FIELD_NAME_POSTFIX_DIR_st;
        ...
        cv_bridge::CvImageConstPtr cvPtr_c = cv_bridge::toCvCopy(srv_c.response.field_c,
 ...
 * ---------------------------------------------------------
*/
bool GetDirection::checkRFID_b(aal_potential_field_navigation::check_rfid &srv_c, uint64_t currentRFID_ui,
 ...
 * -----------------------------------------------------------------
*/
bool GetDirection::generateFieldKey_b(string &fieldKey_st, uint64_t targetRFID_ui, uint64_t currentRFID_ui,
                                      uint64_t initialRFID_ui, uint64_t lastRFID_ui) {
