...
void process(const nav_msgs::OdometryConstPtr &odom_pixel) {
    ...
    tf::Vector3 vec3(odom_pixel->pose.pose.position.x * meterPerPixel, odom_pixel->pose.pose.position.y * meterPerPixel,
                     0);
...
int main(int argc, char *argv[]) {
    ...
    node.param<string>("amiro_pseudo_metric_odom_publisher_topic", amiroPseudoMetricOdomPublisherTopic,
                       "/amiro1/odom/pseudo_metric");
    ...
    ROS_INFO("[%s] amiro_pixel_odom_listener_topic: %s", ros::this_node::getName().c_str(),
             amiroPixelOdomListenerTopic.c_str());
    ROS_INFO("[%s] amiro_pseudo_metric_odom_publisher_topic: %s", ros::this_node::getName().c_str(),
             amiroPseudoMetricOdomPublisherTopic.c_str());
